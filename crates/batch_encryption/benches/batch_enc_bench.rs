...
/// Batch sizes used for correctness tests.
const CORRECTNESS_ELL: &[usize] = &[1, 2, 4, 8];

/// Batch sizes used for performance benchmarks.
...
fn correctness_pre_decrypt_rejects_tampered() {
    ...
    let mut cts: Vec<Ciphertext<Bls12_381>> = (0..4)
        .map(|_| encrypt::<Bls12_381, Sha256Oracle>(&out.ek, &PairingOutput::rand(rng), rng))
        .collect();
    cts[2].ct1 = (<Bls12_381 as Pairing>::G1::from(cts[2].ct1)
        + <Bls12_381 as Pairing>::G1::generator())
    .into_affine();
    assert!(pre_decrypt::<Bls12_381, Sha256Oracle>(&out.sk, &out.ek, &cts, rng).is_none());
}
...
fn correctness_zero_message() {
    ...
    let cts: Vec<_> = messages
        .iter()
        .map(|m| encrypt::<Bls12_381, Sha256Oracle>(&out.ek, m, rng))
        .collect();
    let sbk = pre_decrypt::<Bls12_381, Sha256Oracle>(&out.sk, &out.ek, &cts, rng).unwrap();
...
fn correctness_tests(c: &mut Criterion) {
    let mut group = c.benchmark_group("batch_enc_correctness");
...
fn bench_setup(c: &mut Criterion) {
    let mut group = c.benchmark_group("batch_enc_setup");
    group.sample_size(10);
    for &ell in PERF_ELL {
...
fn bench_encrypt(c: &mut Criterion) {
    let mut group = c.benchmark_group("batch_enc_encrypt");
    group.sample_size(10);
    let rng = &mut test_rng();
    let out = setup::<Bls12_381>(4, rng);
    let m = PairingOutput::<Bls12_381>::rand(rng);
    group.bench_function("bls12_381", |b| {
        b.iter(|| encrypt::<Bls12_381, Sha256Oracle>(&out.ek, &m, &mut test_rng()))
    });
    group.finish();
}

fn bench_pre_decrypt(c: &mut Criterion) {
    let mut group = c.benchmark_group("batch_enc_pre_decrypt");
    group.sample_size(10);
    for &ell in PERF_ELL {
        let rng = &mut test_rng();
        let (ek, sk, _, _, cts, _) = full_pipeline::<Bls12_381>(ell, rng);
        group.bench_with_input(BenchmarkId::new("bls12_381", ell), &ell, |b, _| {
            b.iter(|| {
                pre_decrypt::<Bls12_381, Sha256Oracle>(&sk, &ek, &cts, &mut test_rng())
            })
        });
    }
    group.finish();
...
fn bench_decrypt(c: &mut Criterion) {
    let mut group = c.benchmark_group("batch_enc_decrypt");
    group.sample_size(10);
    for &ell in PERF_ELL {
        let rng = &mut test_rng();
